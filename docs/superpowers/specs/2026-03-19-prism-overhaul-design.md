# Loom Plugin Overhaul — Design Spec v0.2.0

## Overview

Google Stitch가 2026-03-18 대규모 업데이트(AI-Native Infinite Canvas, Gemini 3, 공식 Agent Skills/SDK 출시)를 단행함에 따라, loom 플러그인을 전면 개편한다.
...
- **commands/loom.md**: Phase 4-5 실행 순서에서 `create_design_system`/`apply_design_system` 직접 호출 제거, DESIGN.md 워크플로우로 교체. 신규 서브커맨드(`analyze`, `design-md`, `remotion`) 추가. 인증 플로우를 STITCH_API_KEY 우선으로 변경.
...
```
/loom analyze → 상세 프롬프트 사전 준비 (크레딧 소비 없음)
/loom design  → PRO로 전체 화면 원샷 생성
...
| 6 | remotion | Remotion 기반 워크스루 영상 생성 | /loom remotion 커맨드 |
...
| Command | 변경 | 설명 |
|---------|------|------|
| `/loom analyze [app]` | **신규** | 코드+시뮬레이터 분석 → Feature 분리 → 상세 프롬프트 → analysis.md 산출 |
| `/loom design [feature]` | 업데이트 | analysis.md를 입력으로 받아 PRO 생성 → FLASH 수정 |
| `/loom implement [feature]` | 업데이트 | react-components/shadcn-ui 참조 |
| `/loom verify-loop` | 유지 | 변경 없음 |
| `/loom code-verify-loop` | 유지 | 변경 없음 |
| `/loom cancel-loop` | 유지 | 변경 없음 |
| `/loom status` | 유지 | 변경 없음 |
| `/loom list` | 유지 | 변경 없음 |
| `/loom create` | 업데이트 | 새 deviceType/modelId 지원 |
| `/loom edit` | 업데이트 | 새 deviceType/modelId 지원 |
| `/loom variants` | 업데이트 | variantOptions 전체 스키마 지원 |
| `/loom theme` | 업데이트 | MCP 도구 복구 시까지 DESIGN.md 워크플로우로 전환 |
| `/loom export` | 유지 | 변경 없음 |
| `/loom design-md` | **신규** | DESIGN.md 생성/업데이트. 입력: 프로젝트 컨텍스트(Stitch 프로젝트 또는 URL). 산출물: `.loom/DESIGN.md`. 실행: `official/design-md/` 워크플로우 참조 |
| `/loom remotion` | **신규, P2** | 워크스루 영상 생성. 입력: Stitch 프로젝트. 산출물: Remotion 프로젝트 + MP4. 실행: `official/remotion/` 워크플로우 참조. 초기 구현에서 제외 가능 |

## Pipeline 0: Analyze (`/loom analyze`) — 신규

### 목적

코드와 실제 실행 화면을 분석하여, Feature별 상세 Stitch 프롬프트를 포함한 분석 문서를 산출한다. `/loom design`의 입력 자료가 된다.
...
## Pipeline 1: Code→Design (`/loom design`) 업데이트

### 변경된 흐름

```
/loom analyze → docs/plans/{date}-{app}-analysis.md 산출
                    ↓
/loom design [feature] → analysis.md에서 해당 Feature의 프롬프트 로드
                         → Phase 4부터 실행 (Phase 1-3 생략)
```

`/loom design`에 analysis.md가 없으면 기존 Phase 1-3 실행 (하위 호환).
...
## Pipeline 2: Design→Code (`/loom implement`) 업데이트
...
| Phase 1-3에서 분석+프롬프트 | `/loom analyze` 별도 분리 | 코드+시뮬레이터 분석 → 상세 프롬프트 사전 준비 |
| 없음 | /loom analyze | 코드+시뮬레이터 분석 커맨드 |
| 없음 | /loom design-md | DESIGN.md 전용 커맨드 |
| 없음 | /loom remotion | 워크스루 영상 커맨드 |
