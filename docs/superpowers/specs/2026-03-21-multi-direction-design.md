...
enhance-prompt가 Direction 키워드를 감지하여 해당 방향에 맞게 UI/UX 용어, 분위기, 색상 체계를 자동 증폭.
...
### Feature × Direction 순차 처리 (all 모드)

`/loom design all --directions 3`일 때:

```
Feature 1 × Direction A → 검증 → VERIFIED
Feature 1 × Direction B → 검증 → VERIFIED
Feature 1 × Direction C → 검증 → VERIFIED
Feature 2 × Direction A → 검증 → VERIFIED
...
...
- `project_ids`: Direction별 프로젝트 ID (파이프 구분)
...
DESIGN_VERIFIED 감지 시:
  ...
  3. 있으면 → Direction 전환:
     - 다음 Direction 있음 → direction_index++, phase→generation, block
     - 마지막 Direction → completed_directions 업데이트
       - all 모드 + 다음 Feature → Feature 전환, direction_index 리셋
...
| 파일 | 이유 |
|---|---|
| `skills/loom/references/sheet-template.md` | Direction은 analysis.md 내부에 저장 |
