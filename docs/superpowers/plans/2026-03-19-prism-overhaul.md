# Loom Plugin v0.2.0 Overhaul Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stitch 플러그인을 v0.2.0으로 전면 개편 — 공식 스킬 포크, MCP 도구 업데이트, analyze 파이프라인 신설, 크레딧 전략 통합.

**Architecture:** 공식 Agent Skills 7개를 references/official/에 디렉토리째 포크하고, MCP 도구 참조를 8개로 축소 업데이트하며, /loom analyze 커맨드를 신설하여 코드+시뮬레이터 분석 → 상세 프롬프트 산출 파이프라인을 추가한다. 기존 design/implement 파이프라인은 골격을 유지하면서 공식 스킬 참조, PRO/FLASH 크레딧 전략, DESIGN.md 폴백을 통합한다.

**Tech Stack:** Claude Code Plugin (markdown-based), Stitch Remote MCP (HTTP), gcloud ADC / STITCH_API_KEY

**Spec:** `docs/superpowers/specs/2026-03-19-loom-overhaul-design.md`
...
| File | Responsibility |
|------|---------------|
| `skills/loom/references/official/` (7 dirs) | 공식 stitch-skills 포크 (원본 보존) |
| `skills/loom/references/workflows-analyze.md` | /loom analyze 파이프라인 (신규) |
| `skills/loom/references/sdk.md` | @google/stitch-sdk 레퍼런스 (P2) |
...
| `skills/loom/references/tools.md` | 14개 → 8개, 새 파라미터, deprecated 표기 |
| `skills/loom/SKILL.md` | 인증 플로우, Pattern 3 교체, analyze 참조 |
| `skills/loom/references/prompting.md` | enhance-prompt 참조 추가 |
| `skills/loom/references/workflows-design.md` | analysis.md 통합, PRO/FLASH, DESIGN.md 폴백 |
| `skills/loom/references/workflows-implement.md` | react-components/shadcn-ui 참조, get_screen name 필드 |
| `skills/loom/references/sheet-template.md` | analysis 산출물 템플릿 추가 |
| `commands/loom.md` | analyze/design-md/remotion 서브커맨드, 인증/모델 업데이트 |
...
| `skills/loom/references/cdp-iframe-helper.md` | chrome-viewer 패턴 유지 |
...
**Files:**
- Create: `skills/loom/references/official/stitch-design/`
- Create: `skills/loom/references/official/stitch-loop/`
- Create: `skills/loom/references/official/design-md/`
- Create: `skills/loom/references/official/enhance-prompt/`
- Create: `skills/loom/references/official/react-components/`
- Create: `skills/loom/references/official/remotion/`
- Create: `skills/loom/references/official/shadcn-ui/`
...
```bash
mkdir -p skills/loom/references/official
...
```bash
ls -la skills/loom/references/official/
...
```bash
git add skills/loom/references/official/
...
**Files:**
- Modify: `skills/loom/references/tools.md`

- [ ] **Step 1: 현재 tools.md 읽기**

`Read: skills/loom/references/tools.md` 전체 내용 확인.
...
`/loom analyze` 요청 시 아래 5단계를 따른다. design 파이프라인의 입력 자료를 산출한다.
...
`/loom design` 실행 시 `docs/plans/*-analysis.md`가 존재하면:
...
**Files:**
- Create: `skills/loom/references/workflows-analyze.md`

- [ ] **Step 1: 스펙의 Pipeline 0 섹션 참조**

`docs/superpowers/specs/2026-03-19-loom-overhaul-design.md`의 "Pipeline 0: Analyze" 섹션을 기반으로 작성.
...
Phase 1-5 execution guide for `/loom analyze [app]`.
...
```bash
git add skills/loom/references/workflows-analyze.md
...
**Files:**
- Modify: `skills/loom/references/workflows-design.md`
...
**Files:**
- Modify: `skills/loom/references/workflows-implement.md`
...
**Files:**
- Modify: `skills/loom/references/prompting.md`
...
2. **시뮬레이터 분석 활용**: `/loom analyze`로 사전 분석 → 현재 앱의 색상/분위기를 프롬프트에 반영
...
Use this template for `/loom analyze` pipeline. Save to `docs/plans/{date}-{app}-analysis.md`.
...
```markdown
| `analyze` | `/loom analyze [app]` | 코드+시뮬레이터 분석 → Feature별 상세 프롬프트 → analysis.md 산출 |
| `design-md` | `/loom design-md` | DESIGN.md 생성/업데이트 (official/design-md 참조) |
| `remotion` | `/loom remotion` | **P2** 워크스루 영상 생성 (official/remotion 참조) |
```

- [ ] **Step 3: /loom analyze 섹션 추가**

Usage 테이블 뒤, `/loom design` 섹션 앞에:
```markdown
## `/loom analyze [app]` — 코드+시뮬레이터 분석
...
- [ ] **Step 4: /loom design 섹션 업데이트**

다음 변경사항을 `/loom design` 섹션에 반영:
...
- [ ] **Step 5: /loom implement 섹션 업데이트**

다음 변경사항을 `/loom implement` 섹션에 반영:
```markdown
...
- [ ] **Step 6: /loom design-md 섹션 추가**

```markdown
## `/loom design-md` — DESIGN.md 생성/업데이트

1. `Read: references/official/design-md/` → 실행 절차 참조
2. 입력: Stitch 프로젝트 컨텍스트 또는 URL
3. 산출물: `.loom/DESIGN.md`
```

- [ ] **Step 6: /loom remotion 섹션 추가**

```markdown
## `/loom remotion` — 워크스루 영상 생성 (P2)
...
- [ ] **Step 7: /loom create, edit, variants 업데이트**

각 섹션에 새 deviceType/modelId 지원 명시.

- [ ] **Step 8: /loom theme 업데이트**

```markdown
## `/loom theme` — 디자인 시스템

1. `create_design_system` 호출 시도
2. **MCP 도구 없을 시**: DESIGN.md 워크플로우로 전환 (`/loom design-md` 참조)
...
**Files:**
- Create: `skills/loom/references/sdk.md`
...
```bash
git add skills/loom/references/sdk.md
...
```bash
find skills/loom/references/ -type f | sort
```
...
```bash
grep -r "references/" skills/loom/ --include="*.md" | grep -v official/ | head -30
```
