...
```
A1    코드 심층 분석        ─── Screen State Matrix 7축으로 모든 화면 요소 추출 (grep)
 ↓
A1.5  Deep Functional Scan  ─── 각 화면 파일을 읽어서 버튼/데이터/네비/조건부UI/오버레이 내용 추출
 ↓
A3    Feature 분리          ─── 기능 단위로 그룹핑 (병합 금지, Feature당 15-20개 화면)
 ↓
A4    프롬프트 작성          ─── A1.5 기능 카드 참조, 실제 버튼 라벨/데이터 필드 포함
 ↓
