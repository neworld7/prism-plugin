...
```
A1  코드 심층 분석          ─── Screen State Matrix 6축으로 모든 화면 요소 추출
 ↓
...
| **Screen States** | empty, loading, error, populated | `EmptyState`, `isLoading`, `ErrorWidget` |
| **Overlays** | 모달, 바텀시트, 다이얼로그, 스낵바 | `showModalBottomSheet`, `AlertDialog` |
| **Interaction Modes** | 편집, 드래그, 스와이프, 검색 | `_isEditMode`, `ReorderableListView` |
| **System States** | 스플래시, 권한, 오프라인, 강제 업데이트 | `SplashScreen`, `requestPermission` |
| **Navigation** | 탭, 네비게이션 구조 | `BottomNavigationBar`, `TabBar` |
...
Screen State Matrix 기반으로 4개 축에서 화면을 생성합니다:

| 축 | 개수 | 예시 |
|----|------|------|
| **Primary Screens** | 5~7개 | 책장, 책 상세, 검색 |
| **Screen States** | 3~5개 | `서재 (empty)`, `서재 (skeleton)`, `서재 (error)` |
...
```
서재 (책장, 그리드 뷰)          ← Primary
서재 (empty)                    ← State: empty
서재 (skeleton)                 ← State: loading
정렬/필터 바텀시트               ← Overlay
...
- **hex 코드, px 값, 특정 폰트명 포함 금지** — 디자인 토큰은 enhance-prompt가 자동 주입
