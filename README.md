...
```
A1    코드 심층 분석        ─── Screen State Matrix 7축 grep (화면/상태/오버레이 위치)
 ↓
A1.5  Deep Functional Scan  ─── 각 화면 파일 Read → 버튼/데이터/네비/조건부UI/오버레이 내용
 ↓
A1.6  Navigation Flow Graph ─── 라우터 파일 Read → 화면 간 이동 맵, 탭 구조, 인증 가드
 ↓
A1.7  Data Model Scan       ─── 엔티티/모델 Read → 필드 목록, 타입, enum, 관계
 ↓
A1.9  String & Copy         ─── l10n/코드에서 실제 한국어 텍스트 수집 → 앱 보이스/톤 파악
 ↓
A1.10 Scroll & Layout       ─── 화면별 스크롤 구조 감지 (SliverAppBar, TabBar, PageView 등)
 ↓
A1.11 External Packages     ─── UI 영향 패키지 감지 (차트, 카메라, 지도, 결제 등)
 ↓
A3    Feature 분리          ─── 기능 단위로 그룹핑 (병합 금지, Feature당 15-20개 화면)
 ↓
A4    프롬프트 작성          ─── A1~A1.11 전체 참조, 기능적으로 완벽한 프롬프트
 ↓
...
| 버전 | 변경 |
|------|------|
| 3.5.1 | A1.9 String & Copy Inventory (실제 한국어 텍스트 수집, 앱 보이스/톤 파악), A1.10 Scroll & Layout Architecture (SliverAppBar/TabBar/PageView 감지), A1.11 External Package Detection (차트/카메라/결제 등 UI 영향 패키지) |
| 3.5.0 | **Analyze 파이프라인 대폭 강화**: A1.6 Navigation Flow Graph (라우터 분석 → 화면 간 이동 맵), A1.7 Data Model Scan (엔티티 구조 → 필드/타입/관계), A1.8 Theme & Asset Scan (색상/폰트/아이콘/간격 패턴). Flutter/React/Next.js 범용 |
| 3.4.3 | **A1.5 Deep Functional Scan 추가** — grep만으로는 기능적 의미 파악 불가. 각 화면 파일을 읽어 버튼/데이터/네비/조건부UI/오버레이 내용을 추출하는 단계 신설. A4 프롬프트에 A1.5 기능 카드 참조 의무화 |
