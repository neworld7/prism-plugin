...
| `export` | `/prism export <feature\|all>` | Stitch 디자인 → Figma 내보내기 |
...
```
A1   Code Pattern Scan      ─── Screen State Matrix 7축 grep (화면/상태/오버레이 위치)
 ↓
A2   Functional Scan        ─── 각 화면 파일 Read → 버튼/데이터/네비/조건부UI/오버레이 내용
 ↓
A3   Navigation Flow        ─── 라우터 파일 Read → 화면 간 이동 맵, 탭 구조, 인증 가드
 ↓
A4   Data Model             ─── 엔티티/모델 Read → 필드 목록, 타입, enum, 관계
 ↓
A5   String & Copy          ─── l10n/코드에서 실제 한국어 텍스트 수집 → 앱 보이스/톤 파악
 ↓
A6   Layout Architecture    ─── 화면별 스크롤 구조 감지 (SliverAppBar, TabBar, PageView 등)
 ↓
A7   Package Detection      ─── UI 영향 패키지 감지 (차트, 카메라, 지도, 결제 등)
 ↓
A8   Feature Separation     ─── 기능 단위로 그룹핑 (병합 금지, Feature당 15-20개 화면)
 ↓
A9   Prompt Writing         ─── A1~A7 전체 참조, 기능적으로 완벽한 프롬프트
 ↓
A10  Design Preview         ─── 7개 시안 생성 (별도 /prism preview로도 실행 가능)
 ↓
A11  Prompt Enhancement     ─── 공식 스킬로 프롬프트 최적화 + 디자인 토큰 주입
 ↓
A12  Save Outputs           ─── .prism/analysis.md + .prism/prompts.md
```
...
├── .prism/
│   ├── analysis.md                     ← 코드 분석 결과 (A1-A9)
│   ├── prompts.md                      ← 최적화된 프롬프트 (A11)
...
│       └── references/
│           └── workflows.md            ← 전체 워크플로우 상세 (A1-A12, D1-D6)
├── hooks/
...
| **레퍼런스** | `skills/prism/references/workflows.md` | A1-A12, D1-D6 전체 워크플로우 상세 절차 |
...
| 버전 | 변경 |
|------|------|
| 4.6.0 | `/prism export` + `/prism implement` — Stitch→Figma 내보내기 + Figma→Code 반영. Fidelity 3-Level Fallback. Ralplan 합의 설계 |
| 4.0.0 | **Phase 번호 재정리 (A1~A12 순번)**. A1.5/A1.6/.../A4.5 → A1~A12 순차 번호. 제거된 단계(A1.8, A2) 삭제. A8 확장 테이블 + A9 통합 매핑 규칙. 4개 파일 전체 교차참조 업데이트 |
| 3.5.1 | A5 String & Copy (실제 한국어 텍스트 수집, 앱 보이스/톤 파악), A6 Layout Architecture (SliverAppBar/TabBar/PageView 감지), A7 Package Detection (차트/카메라/결제 등 UI 영향 패키지) |
| 3.5.0 | **Analyze 파이프라인 대폭 강화**: A3 Navigation Flow (라우터 분석 → 화면 간 이동 맵), A4 Data Model (엔티티 구조 → 필드/타입/관계). Flutter/React/Next.js 범용 |
| 3.4.3 | **A2 Functional Scan 추가** — grep만으로는 기능적 의미 파악 불가. 각 화면 파일을 읽어 버튼/데이터/네비/조건부UI/오버레이 내용을 추출하는 단계 신설. A9 프롬프트에 A2 기능 카드 참조 의무화 |
| 3.4.2 | **DESIGN SYSTEM (REQUIRED) 블록 필수화** — 텍스트 앵커만으로는 Stitch가 디자인 시스템 미인식. hex/font/style 토큰 직접 주입 패턴으로 전환. "hex 금지" 규칙 폐기. A9 프롬프트 구조, D3 배치 프롬프트, 예시 전면 업데이트 |
...
| 3.2.0 | 공식 프롬프트 가이드 개편, 시뮬레이터 스크린샷 단계 제거, ./DESIGN.md 전환, preview add/list/use/remove |
