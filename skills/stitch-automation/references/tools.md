...
| `modelId` | string | optional | `MODEL_ID_UNSPECIFIED` (기본), `GEMINI_3_1_PRO`, `GEMINI_3_1_FLASH` |
...
- `GEMINI_3_1_PRO` — 최고 품질 (Thinking with 3 Pro)
- `GEMINI_3_1_FLASH` — 빠른 반복 (3.0 Flash)
...
| Daily Redesign Credits | **15** | 매일 리셋 | Redesign (Nano Banana Pro) 모드 전용 |

> **주의**: 이전 문서의 "PRO 50/월, FLASH 350/월"은 구 체계. 현재는 일일 400 크레딧 통합 체계.

MCP에 할당량 조회 도구가 없으므로 rate limit은 생성 시도 중 에러 응답으로 감지.
크레딧 확인: Stitch 웹 → 프로필 → Stitch 설정 → "오늘 사용량".
