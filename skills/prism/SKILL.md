...
Google Stitch AI design tool을 공식 스킬을 통해 오케스트레이션한다. prism은 코드 분석, 파이프라인 흐름 제어, 검증 루프를 담당하고, 디자인 생성/프롬프트 최적화/디자인 시스템은 공식 Stitch 스킬에 위임한다.
...
2. 보존: cp ./DESIGN.md .prism/directions/{name}/DESIGN.md
3. 이후 Feature: .prism/directions/{name}/DESIGN.md → ./DESIGN.md 복원
