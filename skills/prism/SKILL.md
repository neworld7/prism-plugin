---
name: prism
description: "Google Stitch AI design tool orchestrator. Code analysis → design pipeline via official Stitch skills. Use when user mentions stitch, 스티치, prism, /prism, 디자인 파이프라인, 앱 디자인, 화면 디자인, UI 시안, 디자인 시안, 디자인 시스템, design system, stitch.withgoogle.com. Also trigger when user wants to generate, create, or manage design screens for mobile/web apps."
---
...
크레딧 소진으로 중단 시 진행 상황이 자동 저장된다:
- `.claude/prism-design-pipeline.local.md` — 현재 활성 상태
- `.claude/prism-pipelines/{시안이름}.local.md` — 시안별 백업

저장 필드: `design_name`, `completed_features`, `completed_screens`, `current_axis`, `project_id`

**시안 전환 시에도 각 시안의 진행 상황이 독립 보존된다.**
`/prism preview use <name>` → 상태 파일 자동 스왑.
`/prism design resume` → 중단 지점부터 이어서 생성.
