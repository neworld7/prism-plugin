...
크레딧 소진으로 중단 시 상태 파일(`.claude/prism-design-pipeline.local.md`)에 진행 상황이 자동 저장된다.
- `completed_features`: 완료된 Feature 목록
- `completed_screens`: 현재 Feature에서 생성된 화면 목록
- `current_axis`: 중단된 축 (primary/states/overlays/interactions)
- `project_id`: 현재 Stitch 프로젝트 ID

`/prism design resume`로 중단 지점부터 이어서 생성. 계정 전환(`/prism account <name>`) 후에도 동작.
