...
사용자가 제공한 레퍼런스 URL/앱을 **실제로 분석**한다:

```
1. WebFetch로 레퍼런스 사이트 분석:
   → URL이 있으면: WebFetch(url, "Analyze design system: colors, typography, 
     spacing, shadows, component styles, what makes it sophisticated")
   → 앱 이름만 있으면: WebSearch("{앱이름} app UI design screenshot dribbble behance")

2. 분석 결과에서 추출할 요소:
   | 요소 | 추출 내용 |
   |------|----------|
   | 색상 전략 | 배경색, 악센트 사용 빈도, 채도 수준 |
   | 타이포 전략 | 서체, 위계 방식, 볼드 사용 |
   | 공간 전략 | 여백량, 밀도, 카드 vs 플랫 |
   | 장식 수준 | 그림자/보더/아이콘/일러스트 사용 여부 |
   | 색상 절제도 | "UI가 무채색이고 콘텐츠가 색상" vs "UI 자체가 컬러풀" |

3. 사용자에게 분석 요약 공유:
   "replicate.com 분석 결과: 순백 배경, UI는 거의 무채색, 콘텐츠(AI 이미지)가 
    유일한 색상 소스. 타이포 위계만으로 구조를 만듬. 그림자/보더 최소."
```
