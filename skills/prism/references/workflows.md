...
통합 워크플로우 실행 가이드. Analyze (A1-A12) → Design (D1-D6).
...
- A8의 Feature 목록에서 Primary Screen이 가장 많고 다양한 Feature에서 10개 선정
...
**사용자 응답 처리:**
- "1, 3, 5 저장 / 3 활성" → 3개 저장, 3번을 ./DESIGN.md로 복사, A12으로 진행
- "3번을 X로 바꿔주세요" → 해당 프로젝트 재생성 후 재표시
- 전체 재생성 요청 → A10 처음부터 재실행
