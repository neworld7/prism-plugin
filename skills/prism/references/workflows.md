...
### A3: Feature 분리 (심층 — A1~A1.11 전체 통합)

**Goal:** A1~A1.11의 **모든 분석 결과**를 통합하여 Feature 단위로 화면을 분류한다. A1(grep) 결과만이 아니라 A1.5(기능), A1.6(네비), A1.7(데이터), A1.9(텍스트), A1.10(레이아웃), A1.11(패키지)을 모두 반영한 **풍부한 화면 카드**를 생성한다.

> **⚠️ 핵심 (v3.5.1):** A3은 단순한 "화면 이름 목록"이 아니라, A4 프롬프트 작성을 위한 **완전한 화면 명세**다. A3 Output에 정보가 부족하면 A4 프롬프트도 부족해진다.

**Steps:**

1. 화면을 기능 단위(Feature)로 그룹화한다. **Feature 분리 기준:**
   - 앱의 **하단 네비게이션 탭** 또는 **주요 섹션**이 자연스러운 Feature 경계 ← **A1.6 탭 구조 참조**
   ...
   - **A1.11 패키지 기반 화면 추가**: 결제 패키지 → 구독/페이월 화면, 카메라 패키지 → 캡처 화면 등을 해당 Feature에 명시적으로 포함
...
3. **코드에 없더라도 앱에 필수적인 화면은 추가한다:**
   ...
   - 삭제/수정 액션이 있는 화면 ← **A1.5 버튼/액션 참조**: 확인 다이얼로그 필수 추가
   - 검색 기능이 있는 화면 ← **A1.5 인터랙션 참조**: `search-active`, `검색 결과 없음` 필수 추가
   ...
   - 외부 패키지 화면 ← **A1.11 참조**: 차트/카메라/지도/결제 패키지가 있으면 해당 화면을 명시적으로 포함
...
5. 각 Feature의 화면을 **확장 카드 형식**으로 정리한다. A1~A1.11의 분석 결과를 통합하여 A4가 바로 참조할 수 있는 수준의 정보를 포함한다.

**Output — 확장 화면 테이블 (A1~A1.11 통합):**
...
| 5 | 메인 목록 (empty) | Data: empty | 고정 (스크롤 없음) | CTA 1개 | "아직 등록된 항목이 없어요" | A1.9 |
| 6 | 메인 목록 (partial) | Data: partial | 그리드 (1-2개만) | — | — | 디자인 필수 |
| 7 | 메인 목록 (skeleton) | Data: skeleton | CustomScrollView | 시머 플레이스홀더 | — | 디자인 필수 |
| 8 | 검색 (결과 없음) | Data: empty | 고정 | — | "검색 결과가 없습니다" | A1.9 |
| 9 | 상세 (error) | Data: error | 고정 | 재시도 버튼 | "불러올 수 없어요" | A1.9 |
| 10 | 정렬 바텀시트 (최근/제목/저자순) | Overlay | 바텀시트 | 3개 정렬 옵션 + 적용 버튼 | — | A1.5 |
| 11 | 삭제 확인 ("이 항목을 삭제하시겠어요?") | Overlay | 다이얼로그 | 확인/취소 | "삭제하시겠어요?" | A1.5+A1.9 |
| 12 | 상태 변경 (진행 중/대기/완료/중단) | Overlay | 바텀시트 | 4개 enum 옵션 | — | A1.5+A1.7 |
| 13 | 메인 목록 (edit mode) | Interaction | 그리드 + 상단 액션 바 | 다중 선택 + 삭제/이동 | "N건 선택됨" | 코드 |
| 14 | 메인 목록 (search active) | Interaction | 검색바 확장 + 필터 | 실시간 필터링 | — | 코드 |
| 15 | 메인 목록 (비로그인) | Auth | 고정 | CTA 1개 | "로그인하고 시작해보세요" | A1.9 |
```
...
|----|------|------|
| **화면** | A1 (grep) | 화면 이름 + 상태 |
| **축** | A3 (Matrix 7축) | 어떤 축에 해당하는지 |
| **레이아웃** | A1.10 | SliverAppBar/TabBar/PageView/ListView 등 |
| **주요 기능** | A1.5 | 버튼, 인터랙션, 네비게이션 목적지 |
| **핵심 텍스트** | A1.9 + A1.5 | 실제 한국어 문구 (빈 상태, 에러, 버튼 라벨) |
| **소스** | A1~A1.11 | 어디서 도출되었는지 (코드/디자인 필수/분석 단계) |

> **A3 → A4 연결:** A4는 이 확장 테이블의 **각 행**을 하나의 Screen 프롬프트로 변환한다. 테이블에 정보가 충분하면 A4에서 추가 조사가 불필요하다.
