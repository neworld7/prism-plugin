---
name: loom
description: "Google Stitch AI design tool automation. Design pipeline, screen CRUD, design systems, variants. Use when user mentions stitch, 스티치, stitch.withgoogle.com, Stitch 디자인, Stitch MCP, loom, /loom."
---
...
| 5 | 산출물 | .loom/{date}-{app}-analysis.md 작성 |
...
|-------|------|------|
| 1 | 코드 분석 | Glob/Grep으로 화면·인터랙션·상태 전수 조사 |
| 2 | 디자인 시트 작성 | `.loom/` 에 코드 vs 디자인 비교 문서 |
...
`/loom design` 실행 시 `.loom/*-analysis.md`가 존재하면:
...
`/loom design all` 실행 시 모든 Feature를 한꺼번에 처리하지 않고, **Feature별로 순차 처리**한다:
...
2. tool_not_found 에러 → DESIGN.md 워크플로우로 폴백:
   - 공식 design-md 스킬 사용 또는 수동으로 .loom/DESIGN.md 생성
...
| Design sheet template | `references/sheet-template.md` |
